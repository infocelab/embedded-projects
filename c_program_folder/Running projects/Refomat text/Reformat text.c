...
Head pointer has no data
The list is empty if right pointer of head is equal to head
+-----++-----+
| || |
| VV |
+--- +----+----+ ---+
...
Be careful to update the pointers in right order
+------------------------------------------------+
| +--------------------------------------|--+
| V V |
+--- +----+----+ ---> +----+----+ -- ... -> +----+----+
h_l ------> | sx | dx | <--- | sx | dx | <- ... -- | sx | dx |
+----+----+ +----+----++----+----+
| no data | | data|| data|
+---------+ +---------++---------+
*/
...
/*--------------------------------------------------------*/
/*
Scan linked list forward
use a_l as current pointer
*/
int gets_l(len,buf)
int *len;
char *buf;
...
/*---------------------------------------------------------*/
/*
Append one char in ASCIIZ string
*/
...
ex.: AAAAAAAAAAxxxxxxxBBBBBBBBBB before
ldelete(27 (data len),
char * of data,
11 (start position),
7 (number of char to remove));
AAAAAAAAAABBBBBBBBBB after
*/
...
/*--------------------------------------------------------*/
/*
Get a token (word) from data buffer
*/
